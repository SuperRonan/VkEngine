...
#define IMPORT_IMGUI_EXTERN_TEMPLATES(T, ST, FT) \
	...
	InputRangeRes SliderRangeEx(const char* label, ImGuiDataType data_type, void* range, const void* bounds, const void* p_len_bounds, const char* format, ImGuiSliderFlags flags)
	{
		...
		IdHelper id_helper(label);
		ImGuiContext& g = *GImGui;
		const ImGuiStyle& style = g.Style;
		const ImGuiID ids[3] = {window->GetID(0), window->GetID(1), window->GetID(2)};
		const ImGuiID id = ids[0];
		const float w = CalcItemWidth();
		...
		ItemSize(total_bb, style.FramePadding.y);
		if (!ItemAdd(total_bb, ids[0], &frame_bb, temp_input_allowed ? ImGuiItemFlags_Inputable : 0))
			return false;

		const ImGuiDataTypeInfo* data_type_info = DataTypeGetInfo(data_type);
		void* p_min = range;
		void* p_max = static_cast<char*>(range) + data_type_info->Size;
		const void* bounds_min = bounds;
		const void* bounds_max = static_cast<const char*>(bounds) + data_type_info->Size;

		// Default format string when passing NULL
		if (format == NULL)
			format = data_type_info->PrintFmt;

		const bool hovered = ItemHoverable(frame_bb, ids[0], g.LastItemData.ItemFlags);
		bool temp_input_is_active = temp_input_allowed && TempInputIsActive(ids[0]);
		if (!temp_input_is_active)
		{
			// Tabbing or Ctrl+Click on Drag turns it into an InputText
			const bool clicked = hovered && IsMouseClicked(0, ImGuiInputFlags_None, id);
			const bool double_clicked = (hovered && g.IO.MouseClickedCount[0] == 2 && TestKeyOwner(ImGuiKey_MouseLeft, ids[0]));
			const bool make_active = (clicked || double_clicked || g.NavActivateId == id);
			if(make_active)
			{
				ImRect left_bb = frame_bb;
				left_bb.Max.x /= 2;
				ImRect right_bb = frame_bb;
				right_bb.Min.x = left_bb.Max.x + 1;
				if (left_bb.Contains(g.IO.MousePos))
				{
					p_data = p_min;
				}
				else
			...
			{
				// Will be determined by behaviour
				SliderRangeGlobals::_g.index = SliderRangeGlobals::None;
			}
			// Store initial value (not used by main lib but available as a convenience but some mods e.g. to revert)
			if (make_active)
				memcpy(&g.ActiveIdValueOnActivation, p_data, data_type_info->Size);
		...
		// Behavior
		SliderRangeParams behaviour_out;
		const bool value_changed = SliderRangeBehavior(frame_bb, id, data_type, p_min, p_max, bounds_min, bounds_max, format, flags, &behaviour_out);
		if (value_changed)
			MarkItemEdited(id);

		const ImRect& inter_bb = behaviour_out.boxes[static_cast<size_t>(SliderRangeParams::Inter)];
		const ImRect& min_bb = behaviour_out.boxes[static_cast<size_t>(SliderRangeParams::Min)];
		const ImRect& max_bb = behaviour_out.boxes[static_cast<size_t>(SliderRangeParams::Max)];


		// Render grabs
		window->DrawList->AddRectFilled(inter_bb.Min, inter_bb.Max, GetColorU32(behaviour_out.active_index == SliderRangeParams::Inter ? ImGuiCol_SliderGrabActive : ImGuiCol_SliderGrab, 0.5f), style.GrabRounding, ImDrawFlags_None);
		window->DrawList->AddRectFilled(min_bb.Min, min_bb.Max, GetColorU32(behaviour_out.active_index == SliderRangeParams::Min ? ImGuiCol_SliderGrabActive : ImGuiCol_SliderGrab), style.GrabRounding, ImDrawFlags_None);
		window->DrawList->AddRectFilled(max_bb.Min, max_bb.Max, GetColorU32(behaviour_out.active_index == SliderRangeParams::Max ? ImGuiCol_SliderGrabActive : ImGuiCol_SliderGrab), style.GrabRounding, ImDrawFlags_None);
		...
		value_buf_end = value_buf + DataTypeFormatString(value_buf, IM_COUNTOF(value_buf), data_type, p_min, format);
		if (g.LogEnabled)
			LogSetNextTextDecoration("{", "");
		RenderTextFit(min_bb, value_buf, value_buf_end);

		value_buf_end = value_buf + DataTypeFormatString(value_buf, IM_COUNTOF(value_buf), data_type, p_max, format);
		if (g.LogEnabled)
			LogSetNextTextDecoration(": ", "}");
		RenderTextFit(max_bb, value_buf, value_buf_end);
